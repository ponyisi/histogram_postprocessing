...
Description: Testing
        ...
        chk = subprocess.run(f"python -m histgrinder.engine example.root example.root -c {configfile.name} --prefix prefix",
                             shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        print(chk.stdout)
        assert (b'Function histgrinder.example.transform_function_divide_ROOT gave a return value'
                b' which is not a Mapping but VariableOutput functions must do so.' in chk.stdout)
        with pytest.raises(CalledProcessError):
            chk.check_returncode()
